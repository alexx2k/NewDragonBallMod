...
public class GetEntityLookingAtProcedure {
	@SubscribeEvent
	...
	private static Entity execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (!world.getEntitiesOfClass(Mob.class, AABB.ofSize(new Vec3(lookX, lookY, lookZ), 4, 4, 4), e -> true).isEmpty()) {
			displayentity = (Entity) world.getEntitiesOfClass(Mob.class, AABB.ofSize(new Vec3(lookX, lookY, lookZ), 4, 4, 4), e -> true).stream().sorted(new Object() {
			...
			displayentity = (Entity) world.getEntitiesOfClass(Player.class, AABB.ofSize(new Vec3(lookX, lookY, lookZ), 4, 4, 4), e -> true).stream().sorted(new Object() {
				Comparator<Entity> compareDistOf(double _x, double _y, double _z) {
					return Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_x, _y, _z));
				}
			}.compareDistOf(lookX, lookY, lookZ)).findFirst().orElse(null);
		} else {
			displayentity = (Entity) world.getEntitiesOfClass(Player.class, AABB.ofSize(new Vec3(x, y, z), 4, 4, 4), e -> true).stream().sorted(new Object() {
				Comparator<Entity> compareDistOf(double _x, double _y, double _z) {
					return Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_x, _y, _z));
				}
